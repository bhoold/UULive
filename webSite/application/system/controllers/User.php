...
class User extends MY_Controller {
	...
	public function add()
	{
		...
		if($this->input->method() == 'post'){
			$post = $this->input->post(array('username', 'password', 'email', 'sex', 'state'));

			$this->form_validation->reset_validation();
			if($this->form_validation->run() !== FALSE){
				if($this->userModel->get(array('username'=>$post['username']))){
					$this->message->enqueueMessage('账号重复!', 'error');
				}elseif($post['email'] && $this->userModel->get(array('email'=>$post['email']))){
					$this->message->enqueueMessage('电子邮箱重复!', 'error');
				}else{
					$post['password'] = password($post['password']);
					$post['regip'] = $this->input->ip_address();
					$post['createbyadmin'] = 1;
	...
	public function edit($uid = 0)
	{
		...
		if($uid){
			if($user = $this->userModel->get(array('id'=>$uid))){
				...
				if($this->input->method() == 'post'){
					$post = $this->input->post(array('password', 'email', 'sex', 'state'));

					$this->form_validation->reset_validation();
					if($this->form_validation->run() !== FALSE){
						$flag = TRUE;
						if($post['email']){
							$existUser = $this->userModel->get(array('email'=>$post['email']));
							if($existUser && $existUser['id'] != $uid){
								$this->message->enqueueMessage('电子邮箱重复!', 'error');
								$flag = FALSE;
							}
						}
